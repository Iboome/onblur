...
Ext.extend(MenuPanel, Ext.Panel, {
    makeMenu3: function (index, nowItem) {
        ...
        var a = this;
        Ext.Ajax.request({
            url: "index.php?app=main&func=config&action=getMainPanelTreeData2",
            method: "GET",
            success: function (c) {
                var b = Ext.decode(c.responseText);
    ...
    MainPanel.superclass.constructor.call(this, {
        ...
        listeners: {
            afterrender: function (a) {
                ...
                $("#CNOA_MAIN_PANEL .x-tab-edge .x-tab-strip-text").html("常用").powerFloat({
                    target: [],
                    position: "4-1",
                    width: 180,
                    targetMode: "list",
        ...
        items: [{
            ...
            bodyStyle: "padding: 5px 0 0 5px;",
            autoLoad: {
                url: "index.php?action=index&ajax=1&parentid=docs-CNOA_WELCOM_PANEL",
                scripts: true,
                scope: this,
                nocache: true
            }
...
function main_init_desktop() {
    ...
    if (CNOA.systemTitleShowType == 0) {
        t = "<a href='javascript:void(0);' style='background-image: url(" + CNOA.config.file + "/webcache/logo/" + CNOA_USER_THEME + ".gif);' id='CNOA_LOGO' onclick='CNOA.RefreshWindow();' ext:qtip='" + lang("clickToRefresh") + "'></a>"
    } else {
    ...
    var q = function () {
        Ext.Ajax.request({
            url: "index.php?app=notice&func=notice&action=todo&task=getAllMenuList",
            method: "POST",
            success: function (z) {
    ...
    var c = new Ext.Panel({
        ...
        tbar: new Ext.Toolbar({
            ...
            items: ["->", "<img id='CNOA_main_framework_ajax_status' src='./resources/images/wait.gif' style='margin-right:15px;display:none;'>", {
...
function createMyInfoPanel() {
    ...
    a += '<div class="lhc_name_dept">----</div>';
    a += '<div class="lhc_email">----</div>';
    a += '<div id="lhc_main">';
    a += '<div class="lhc_drop_down">';
    a += '<div class="lhc_person">个人资料</div>';
    a += '<div class="lhc_person_info">';
    a += '<div class="lhc_edit_info lhc_common_info"><a href="javascript:void(0)">修改我的资料</a></div>';
    a += '<div class="lhc_edit_password lhc_common_info"><a href="javascript:void(0)">密码管理</a></div>';
    a += '<div class="lhc_edit_group lhc_common_info"><a href="javascript:void(0)">自定群组</a></div>';
    a += "</div>";
    if (CNOA.config.scblg.shmh == 1) {
        a += '<div class="lhc_help lhc_common_about"><a href="javascript:void(0)">帮助中心</a></div>'
    }
    a += '<div class="lhc_about lhc_common_about">';
    if (CNOA.config.scblg.shma == 1) {
        a += '<a href="javascript:void(0)">关于软件</a>'
    }
    a += '<span class="lhc_new_version" style="display:none;"></span></div>';
    a += '<div class="lhc_weather">';
    a += '<div class="lhc_city"></div>';
    a += '<div class="lhc_weather_img"><img src="resources/images/weather/b/01.png" width="60" height="60" style="background-color:#4E97FF;" /></div>';
    a += '<div class="lhc_temp"></div>';
    a += '<div class="lhc_weat"></div>';
    a += '<div class="lhc_setting"><a href="javascript:void(0)">天气设置</a></div>';
    a += "</div>";
    a += "</div>";
    a += "</div>";
    a += '<div class="lhc_footer">';
    a += '<div class="lhc_logout lhc_common_footer"><a href="javascript:void(0)">退出</a></div>';
    a += "</div>";
    a += "</div>";
    $("body").append(a);
    ...
    $(".lhc_edit_password a").click(function () {
        mainPanel.loadClass("index.php?app=my&func=info&action=index&task=loadPage&from=passwd", "CNOA_MENU_MY_PASSWD", "密码管理", "icon-cnoa-my-info")
    });
    $(".lhc_edit_group a").click(function () {
        mainPanel.loadClass("index.php?app=my&func=info&action=index&task=loadPage&from=group", "CNOA_MENU_MY_GROUP", "自定义群组", "icon-cnoa-my-info")
    });
        ...
        url: "index.php?act=commonJob&task=checkUpdate", method: "GET", success: function (b) {
            ...
            if (a) {
                if (a.data != "") {
                    ...
                    $(".lhc_new_version").attr("ext:qtip", "OA系统有升级，版本：v" + a.data[0].version)
                } else {
    ...
    init: function () {
        ...
        this.CT = new Ext.Panel({
            bodyStyle: "padding:15px;", border: false, listeners: {
                render: function (a) {
                    Ext.Ajax.request({
                        url: "index.php?app=main&func=system&action=about", method: "GET", success: function (g) {
...
function main_init_load_task() {
    ...
    a.src = CNOA.config.file + "/webcache/task.js?v=" + CNOA_VERSION;
    b.appendChild(a)
}
function show_expire_window() {
    var a = "";
    if (CNOA_VERSION_INFO.keyOutdate) {
        a = "授权信息失效，请联系我们重新获取授权信息[电话:020-85555657]"
    }
    ...
    var g = function () {
        Ext.Ajax.request({
            url: "index.php?task=getonlinecount", method: "GET", success: function (j) {
                ...
                $("#onlinePeople").html("在线人数:[" + k.oluids.length + "]+[" + k.mlnums + "]")
            }
        })
    };
    g();
...
function getServerDate() {
    ...
    Ext.Ajax.request({
        url: "index.php?action=commonJob&act=getServerTime13", method: "POST", success: function (c, b) {
...
Ext.onReady(function () {
    ...
    if (window.location.host == "oa.cnoa.cn") {
    }
    if (CNOA_VERSION_INFO.wrongmkey) {
        CNOA.msg.alert("授权失效，请联系我们重新授权", function () {
            if (CNOA_USER_JOBTYPE == "superAdmin") {
                mainPanel.loadClass("index.php?app=main&func=system&action=regist&modal=1", "CNOA_MENU_SYSTEM_SETUP_REGIST", lang("register"), "icon-key")
            } else {
                CNOA.Logout("index.php?app=main&func=passport&action=logout")
